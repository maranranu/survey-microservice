...
const tasks = new Listr([
  {
    title: 'Remove SurveyController routes',
    task() {
      return replace({
        files: routerPath,
        from: /\s*apiRouter.*SurveyController'\)\);/,
        to: ''
      });
    }
  },
  {
    ...
    task() {
      return replace({
        ...
        from: [
          /\s*const \{(\n.*)+.*app\/survey'\);/,
          /\s*const.*SurveyRepository'\);/,
          /\s*const.*SurveySerializer'\);/,
          /\, Survey: SurveyModel/,
          /\s*surveyRepository.*\}\]/,
          /\,\s*SurveyModel/,
          /\s+createSurvey(.|\n)+.*DeleteSurvey\n/,
          /\s+surveySerializer: SurveySerializer\n/
        ],
        to: ''
      });
    }
  },
  {
    ...
    task() {
      return remove([
        path.join(srcAndTestPath, 'app', 'survey', '**'),
        path.join(srcAndTestPath, 'domain', 'survey', '**'),
        path.join(srcAndTestPath, 'infra', 'survey', '**'),
        path.join(srcAndTestPath, 'interfaces', 'http', 'survey', '**'),
        path.join(srcPath, 'infra', 'database', 'migrate', '*.js'),
        path.join(srcPath, 'infra', 'database', 'seeds', '*.js'),
        path.join(srcPath, 'infra', 'database', 'models', 'Survey.js'),
        path.join(testPath, 'features', 'api', 'survey', '**'),
